...
class UpgradeData implements UpgradeDataInterface
{
    ...
    private function updateAttributesForVersion110($setup)
    {
        ...
        // Change attribute_group_code from ds-giftcard-info to gift-card-information
        if ($attributeSetId = $this->eavSetup->getAttributeSet($this->entityTypeId, 'Default', 'attribute_set_id')) {
            $changeGroup = 'ds-giftcard-info';
            // Find attribute group id with code ds-giftcard-info
    ...
    private function copyFromGiftcardQuoteToOrderTable($connection, $setup)
    {
        $select = $connection->select()
            ->from($setup->getTable('ds_giftcard_quote'));
        ...
        foreach ($quotes as $quote) {
            ...
            if ($orderId = $connection->fetchOne($select)) {
                ...
                $connection->insert(
                    $setup->getTable('ds_giftcard_order'),
                    $data
                );
            }
        }
        ...
        foreach ($objects as $object) {
            foreach ($updateTables as $updateTable => $updateRelationField) {
                $connection->update(
                    ...
                    [
                        'base_ds_giftcard'. $updateSuffix => $object['base_giftcard_amount'],
                        'ds_giftcard'. $updateSuffix => $object['base_giftcard_amount']
                    ],
