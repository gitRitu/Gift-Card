 ...
 */
class OrderInfo extends \Dotsquare\Giftcard\Ui\Component\Form\Field
{
    /**
            ...
            switch ($config['dataScope']) {
                case 'product':
                    $dataSource['data']['product_url'] = $this->getUrl(
                        ...
                        ['id' => $product->getId(), 'dsgcBack' => 'editCode', 'dsgcId' => $dataSource['data']['id']]
                    );
                    $dataSource['data']['product_label'] = $product->getName();
                    $dataSource['data']['product_after'] =
                        '(' . $this->sourceGiftcardType->getOptionText($product->getDsGcType()) . ')';
                    break;
                case 'order':
