...
namespace Dotsquare\Giftcard\Model\Product\Type;
...
use Dotsquare\Giftcard\Model\Source\Entity\Attribute\GiftcardCustomMessage;
use Magento\Catalog\Model\Product;
use Magento\Framework\App\RequestInterface;
use Magento\Framework\Exception\LocalizedException;
use Magento\Catalog\Api\ProductRepositoryInterface;
use Dotsquare\Giftcard\Model\Source\Entity\Attribute\GiftcardType;
use Dotsquare\Giftcard\Model\Email\Sender;
 ...
 * Class Giftcard
 *
 * @package Dotsquare\Giftcard\Model\Product\Type
 */
class Giftcard extends AbstractType
{
    ...
    {
        return
            $this->getAttribute($product, ProductAttributeInterface::CODE_DS_GC_TYPE) == GiftcardType::VALUE_VIRTUAL;
    }
    ...
    {
        return
            $this->getAttribute($product, ProductAttributeInterface::CODE_DS_GC_TYPE) == GiftcardType::VALUE_PHYSICAL;
    }
    ...
    {
        return
            $this->getAttribute($product, ProductAttributeInterface::CODE_DS_GC_TYPE) == GiftcardType::VALUE_COMBINED;
    }
    ...
    public function getAmounts(Product $product)
    {
        ...
        $amountsData = $this->getAttribute($product, ProductAttributeInterface::CODE_DS_GC_AMOUNTS);
    ...
    public function getTemplateOptions(Product $product)
    {
        ...
        $templatesData = $product->getData(ProductAttributeInterface::CODE_DS_GC_EMAIL_TEMPLATES);
    ...
    public function getOrderOptions($product)
    {
        ...
        $resultValidDeliveryDate = $this->isValidDeliveryDate(
            $optionObject->getDsGcDeliveryDate(),
            $optionObject->getDsGcDeliveryDateTimezone()
        );
        if (!$resultValidDeliveryDate['success']) {
            $optionObject->setDsGcDeliveryDate(null);
            $optionObject->setDsGcDeliveryDateTimezone(null);
        }
    ...
    protected function _prepareProduct(DataObject $buyRequest, $product, $processMode)
    {
        ...
        $headlineValue = null;
        $messageValue = null;
        if ($this->headlineIsAllowed($product->getData(ProductAttributeInterface::CODE_DS_GC_CUSTOM_MESSAGE_FIELDS))) {
            $headlineValue = $buyRequest->getData(OptionInterface::HEADLINE);
        }
        if ($this->messageIsAllowed($product->getData(ProductAttributeInterface::CODE_DS_GC_CUSTOM_MESSAGE_FIELDS))) {
            $messageValue = $buyRequest->getData(OptionInterface::MESSAGE);
        }
        $deliveryDateValue = null;
        $deliveryDateTimezoneValue = null;
        if ($product->getData(ProductAttributeInterface::CODE_DS_GC_ALLOW_DELIVERY_DATE)) {
        ...
        $giftcardType = $this->getAttribute($product, ProductAttributeInterface::CODE_DS_GC_TYPE);
    ...
    private function isCustomAmount(DataObject $buyRequest, $product)
    {
        ...
        ) && $product->getData(ProductAttributeInterface::CODE_DS_GC_ALLOW_OPEN_AMOUNT);
    }
    ...
    private function validateAmount(DataObject $buyRequest, $product, $processMode, $amount)
    {
        ...
        if ($this->isCustomAmount($buyRequest, $product)) {
            $minOpenAmount = $product->getData(ProductAttributeInterface::CODE_DS_GC_OPEN_AMOUNT_MIN);
            $maxOpenAmount = $product->getData(ProductAttributeInterface::CODE_DS_GC_OPEN_AMOUNT_MAX);
