...
namespace Dotsquare\Giftcard\Block\Product;

use Dotsquare\Giftcard\Api\Data\OptionInterface;
use Dotsquare\Giftcard\Model\DateTime\FormatConverter;
 ...
 * Class View
 *
 * @package Dotsquare\Giftcard\Block\Product
 */
class View extends \Magento\Catalog\Block\Product\View
{
    ...
    {
        return (bool)$this->getProduct()->getData(ProductAttributeInterface::CODE_DS_GC_ALLOW_OPEN_AMOUNT);
    }
    ...
    {
        if ($this->isAllowOpenAmount()) {
            $value = $this->getProduct()->getData(ProductAttributeInterface::CODE_DS_GC_OPEN_AMOUNT_MIN);
    ...
    {
        if ($this->isAllowOpenAmount()) {
            $value = $this->getProduct()->getData(ProductAttributeInterface::CODE_DS_GC_OPEN_AMOUNT_MAX);
            return $this->priceCurrency->convertAndRound($value);
        }
        return 0;
    }
    ...
    {
        return
            !($this->getProduct()->getData(ProductAttributeInterface::CODE_DS_GC_TYPE) == GiftcardType::VALUE_PHYSICAL)
            && !$this->isSingleDesign();
    }
    ...
    {
        return (bool)$this->getProduct()->getData(ProductAttributeInterface::CODE_DS_GC_ALLOW_DELIVERY_DATE);
    }
    ...
    {
        return (int)$this->getProduct()->getData(ProductAttributeInterface::CODE_DS_GC_DAYS_ORDER_DELIVERY);
    }
    ...
    public function isAllowHeadline()
    {
        return $this->getProduct()->getTypeInstance()->headlineIsAllowed(
            $this->getProduct()->getData(ProductAttributeInterface::CODE_DS_GC_CUSTOM_MESSAGE_FIELDS)
        );
    }
    ...
    {
        return $this->getProduct()->getData(ProductAttributeInterface::CODE_DS_GC_DESCRIPTION);
    }
